...
def check_file_versions():
    ...
    if requirements_path.exists():
        ...
        if not black_match or black_match.group(1) != BLACK_VERSION:
            errors.append(
                f"requirements.txt: Black version mismatch (expected {BLACK_VERSION})"
            )
        ...
        if not flake8_match or flake8_match.group(1) != FLAKE8_VERSION:
            errors.append(
                f"requirements.txt: Flake8 version mismatch (expected {FLAKE8_VERSION})"
            )
    ...
    if precommit_path.exists():
        ...
        expected_versions = [
            ...
            FLAKE8_VERSION,
            BANDIT_VERSION,
            PRE_COMMIT_HOOKS_VERSION,
        ]

        if len(rev_matches) != len(expected_versions):
            errors.append(
                f".pre-commit-config.yaml: Expected {len(expected_versions)} rev entries, found {len(rev_matches)}"
            )
        else:
            for i, (found, expected) in enumerate(zip(rev_matches, expected_versions)):
                if found != expected:
                    errors.append(
                        f".pre-commit-config.yaml: Rev {i+1} version mismatch (found {found}, expected {expected})"
                    )
